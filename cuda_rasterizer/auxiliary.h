...
template <typename T>
__forceinline__ __device__ auto ndc2Pix(T v, int S)
{
    ...
    // float3& p_view
{
    ...
    auto orig_points = std::get<1>(jvp_args_tuple);
    auto viewmatrix = std::get<2>(jvp_args_tuple);
    auto projmatrix = std::get<3>(jvp_args_tuple);
    auto prefiltered = std::get<4>(jvp_args_tuple);
    auto& p_view = std::get<5>(jvp_args_tuple);

    auto p_orig = make_float3(orig_points[3 * idx], 
                              orig_points[3 * idx + 1], 
                              orig_points[3 * idx + 2]);
